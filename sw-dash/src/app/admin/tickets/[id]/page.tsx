...
export default function TicketDetail() {
  ...
  function insertFormat(wrap: string) {
    ...
    const newText = reply.slice(0, s) + wrap + reply.slice(s, e) + wrap + reply.slice(e)
    setReply(newText)
    setTimeout(() => {
      el.focus()
  ...
  const skel = () => (
    <main className="h-screen bg-grid overflow-hidden" role="main">
      <div className="hidden md:flex h-full flex-col p-4">
        <div className="h-4 w-32 bg-zinc-800/40 rounded mb-3" />
        <div className="flex-1 flex gap-4 min-h-0">
          <div className="w-80 flex-shrink-0 flex flex-col gap-4 overflow-y-auto pr-1">
            ...
            <div className="bg-gradient-to-br from-zinc-900/90 to-black/90 border-4 border-amber-900/40 rounded-3xl p-4">
              <div className="h-4 w-28 bg-zinc-800/50 rounded mb-3" />
              <div className="space-y-2">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="h-3 w-full bg-zinc-800/30 rounded" />
                ))}
              </div>
            </div>
          </div>
            ...
            <div className="bg-gradient-to-br from-zinc-900/90 to-black/90 border-4 border-amber-900/40 rounded-3xl h-full flex flex-col">
              <div className="flex-1 p-4 space-y-4">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="w-9 h-9 rounded-full bg-zinc-800/40" />
                    <div className="flex-1">
                      <div className="h-4 w-32 bg-zinc-800/40 rounded mb-2" />
                      <div className="h-3 w-full max-w-md bg-zinc-800/30 rounded" />
                    </div>
                  </div>
                ))}
              </div>
              <div className="border-t-2 border-amber-900/40 p-3">
                <div className="flex gap-2">
                  <div className="h-10 w-10 bg-zinc-800/40 rounded-xl" />
                  <div className="flex-1 h-10 bg-zinc-800/30 rounded-xl" />
                  <div className="h-10 w-16 bg-zinc-800/40 rounded-xl" />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="md:hidden h-full flex flex-col p-3">
        <div className="h-4 w-16 bg-zinc-800/40 rounded mb-2" />
        <div className="bg-gradient-to-br from-zinc-900/90 to-black/90 border-4 border-amber-900/40 rounded-3xl p-3 mb-3 min-h-[120px]">
          <div className="flex justify-between items-start mb-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-zinc-800/40" />
              <div>
                <div className="h-4 w-20 bg-zinc-800/40 rounded mb-1" />
        ...
        <div className="flex-1 min-h-0 bg-gradient-to-br from-zinc-900/90 to-black/90 border-4 border-amber-900/40 rounded-3xl" />
      </div>
    </main>
  )
          ...
          <div className="flex flex-wrap gap-2 text-xs items-center">
            ...
            {ticket.status === 'open' && can(user?.role || '', PERMS.support_edit) && (
              <button
                onClick={closeTicket}
                disabled={closing}
                className="text-red-400 font-mono bg-red-900/20 px-2 py-1 rounded-xl border-2 border-red-700/50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {closing ? '...' : 'close'}
              </button>
            )}
          ...
          {can(user?.role || '', PERMS.support_edit) && (
            <div className="border-t-2 border-amber-900/40 p-2">
              ...
              {Object.keys(macros).length > 0 && (
                <div className="flex gap-1 flex-wrap mb-2">
                  {Object.entries(macros).map(([name, content]) => (
                    <button
                      key={name}
                      onClick={() => setReply(content)}
                      className="text-amber-400 font-mono text-xs bg-amber-900/20 px-2 py-1 rounded border border-amber-700/40 hover:bg-amber-900/40"
                    >
                      !{name}
                    </button>
                  ))}
              ...
              <div className="flex gap-2">
                ...
                {preview ? (
                  <div className="flex-1 bg-black border-2 border-amber-700 rounded-xl px-2 py-2 text-amber-200 font-mono text-sm min-h-[38px]">
                    {reply.trim() ? (
                      <MsgRender text={reply} users={ticket.userMap} />
                    ) : (
                      <span className="text-amber-900/50 text-xs">nothing typed yet</span>
                    )}
                  </div>
                ) : (
                  <input
                ...
                <button
                  ...
                  className="text-amber-400/70 font-mono text-xs bg-amber-900/20 px-2 rounded-xl border-2 border-amber-700/50"
                >
                  {preview ? '✏️' : '👁'}
                </button>
                <button
      ...
      <div className="hidden md:flex h-full flex-col p-4">
        ...
        <div className="flex-1 flex gap-3 min-h-0">
          <div className="w-80 flex-shrink-0 flex flex-col gap-3 overflow-y-auto pr-1">
            <SubmitterCard slackId={ticket.userId} username={ticket.userName} className="!w-full" />
            ...
            <div className="bg-gradient-to-br from-zinc-900/90 to-black/90 border-4 border-amber-900/40 rounded-3xl p-4 shadow-2xl shadow-amber-950/30">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <span
                    className={`inline-block px-2 py-0.5 rounded-xl border-2 font-mono text-xs mt-1 ${ticket.status === 'open' ? 'bg-green-900/30 text-green-400 border-green-700' : 'bg-amber-900/30 text-amber-300/70 border-amber-700'}`}
                  >
                    {ticket.status}
                  </span>
                </div>
            ...
            <div className="bg-gradient-to-br from-zinc-900/90 to-black/90 border-4 border-amber-900/40 rounded-3xl p-4 shadow-2xl shadow-amber-950/30">
              <h3 className="text-amber-400 font-mono text-sm mb-3">actions</h3>
              <div className="space-y-4">
                <div>
                  <div className="text-amber-300/60 font-mono text-xs mb-2">assignees</div>
                  {can(user?.role || '', PERMS.support_edit) ? (
                    <div>
                ...
                {ticket.status === 'open' && can(user?.role || '', PERMS.support_edit) && (
                  <button
                    onClick={closeTicket}
                    disabled={closing}
                    className="w-full bg-red-900/30 border-2 border-red-700 text-red-400 hover:bg-red-900/50 font-mono text-xs px-3 py-1.5 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                <div className="border-t-2 border-amber-900/40 p-3">
                    ...
                    <div className="flex gap-1 flex-wrap mb-2">
                      {Object.entries(macros).map(([name, content]) => (
                        <button
                          key={name}
                          onClick={() => setReply(content)}
                          className="text-amber-400 font-mono text-xs bg-amber-900/20 px-2 py-1 rounded-lg border border-amber-700/40 hover:bg-amber-900/40 transition-colors"
                        >
                          !{name}
                        </button>
                      ))}
                  ...
                  <div className="flex gap-1 mb-1 items-center">
                    <button
                      ...
                      className="text-amber-400/70 font-mono text-xs font-bold bg-amber-900/20 px-2 py-0.5 rounded border border-amber-700/30 hover:bg-amber-900/40"
                    >
                      B
                    </button>
                    <button
                      onClick={() => insertFormat('_')}
                      className="text-amber-400/70 font-mono text-xs italic bg-amber-900/20 px-2 py-0.5 rounded border border-amber-700/30 hover:bg-amber-900/40"
                    >
                      I
                    </button>
                    <button
                      onClick={() => insertFormat('`')}
                      className="text-amber-400/70 font-mono text-xs bg-amber-900/20 px-2 py-0.5 rounded border border-amber-700/30 hover:bg-amber-900/40"
                    >
                      {'`code`'}
                    </button>
                    <button
                      onClick={() => insertFormat('~')}
                      className="text-amber-400/70 font-mono text-xs line-through bg-amber-900/20 px-2 py-0.5 rounded border border-amber-700/30 hover:bg-amber-900/40"
                    >
                      S
                    </button>
                    <button
                      onClick={() => setPreview(!preview)}
                      className="ml-auto text-amber-400/70 font-mono text-xs bg-amber-900/20 px-2 py-0.5 rounded border border-amber-700/30 hover:bg-amber-900/40"
                    >
                  ...
                  <div className="flex gap-2">
                    ...
                    {preview ? (
                      <div className="flex-1 bg-black border-2 border-amber-700 rounded-xl px-3 py-2 text-amber-200 font-mono text-sm min-h-[60px]">
                        {reply.trim() ? (
                          <MsgRender text={reply} users={ticket.userMap} />
                        ) : (
                          <span className="text-amber-900/50 text-xs">nothing typed yet</span>
